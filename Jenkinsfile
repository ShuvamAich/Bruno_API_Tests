                ...
                script {
                    withCredentials([string(credentialsId: 'simple-books-authorization', variable: 'AUTHORIZATION_TOKEN')]) {
                        if (isUnix()) {
                            ...
                            sh 'npx bru run --env "Books Environment" --env-var Authorization=$AUTHORIZATION_TOKEN ci --reporter-html results.html'
                        } else {
                            echo 'Running Bruno API tests...'
                            bat 'npx bru run --env "Books Environment" --env-var Authorization=%AUTHORIZATION_TOKEN% ci --reporter-html results.html'
                        }
                    }
                }
            }
